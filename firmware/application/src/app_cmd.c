...
data_frame_tx_t* cmd_processor_change_adv_name(uint16_t cmd, uint16_t status, uint16_t length, uint8_t *data) {
    uint32_t                err_code;
    ...
    err_code = sd_ble_gap_device_name_set(&sec_mode, (const uint8_t *) data, length);
...
static cmd_data_map_t m_data_cmd_map[] = {
    ...
    {    DATA_CMD_SYSTEM_OFF_ENTER,             NULL,                        cmd_processor_systemoff_enter,               NULL                   },
    {    DATA_CMD_CHANGE_ADV_BLE_NAME,          NULL,                        cmd_processor_change_adv_name,               NULL                   },
};
